        ...
        .forEach(
            fieldGroup -> {
              ...
              if (fieldGroup.getCount(Field.LIBRARY_SELECTION) > 0
                  && fieldGroup.getField(Field.LIBRARY_SELECTION).isValidFieldValueOrFileSuffix())
                manifest.setLibrarySelection(fieldGroup.getValue(Field.LIBRARY_SELECTION));

              if (fieldGroup.getCount(Field.LIBRARY_STRATEGY) > 0
                  && fieldGroup.getField(Field.LIBRARY_STRATEGY).isValidFieldValueOrFileSuffix())
              ...
              if (fieldGroup.getCount(Field.QUALITY_SCORE) > 0) {
                ...
                try {
                  ...
                  error(WebinCliMessage.READS_MANIFEST_READER_INVALID_QUALITY_SCORE_ERROR, qsStr);
                }
              }
              ...
              getFiles(getInputDir(), fieldGroup, ReadsManifestReader.Field.BAM)
                  .forEach(
                      file ->
                          submissionFiles.add(
                              new SubmissionFile(ReadsManifest.FileType.BAM, file)));
              getFiles(getInputDir(), fieldGroup, ReadsManifestReader.Field.CRAM)
                  .forEach(
                      file ->
                          submissionFiles.add(
                              new SubmissionFile(ReadsManifest.FileType.CRAM, file)));
              ...
              fieldGroup.stream()
                  ...
                  .forEach(
                      field -> {
                        ...
                        List<SubmissionFileAttribute> fileAttributes =
                            getAttributes(field).stream()
                                .map(
                                    entry ->
                                        new SubmissionFileAttribute(
                                            entry.getKey(), entry.getValue()))
                                .collect(Collectors.toList());

                        submissionFiles.add(
                            new SubmissionFile(ReadsManifest.FileType.FASTQ, file, fileAttributes));
                      });
            });
  }
