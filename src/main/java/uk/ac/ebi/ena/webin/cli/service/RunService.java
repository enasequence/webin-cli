...
public class RunService extends WebinService {
  private static final Logger log = LoggerFactory.getLogger(RunService.class);

  protected RunService(AbstractBuilder<RunService> builder) {
    super(builder);
  }

  public static class Builder extends AbstractBuilder<RunService> {
    @Override
    public RunService build() {
  ...
  private Run getRun(String runId, String userName, String password) {
    RestTemplate restTemplate = new RestTemplate();

    HttpHeaders headers = new HttpHeaderBuilder().basicAuth(userName, password).build();

    ResponseEntity<RunResponse> response =
        ExceptionUtils.executeWithRestExceptionHandling(
            () ->
                RetryUtils.executeWithRetry(
                    retryContext ->
                        restTemplate.exchange(
                            resolveAgainstWebinRestV1Uri("cli/reference/run/{id}"),
                            HttpMethod.GET,
