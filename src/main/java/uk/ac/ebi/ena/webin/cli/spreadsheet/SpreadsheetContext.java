...
public enum SpreadsheetContext {
  GENOME(
      ...
      "Additionally, primary and binned metagenomes and clinical isolate assembly must have "
          ...
          + " files."),
  TRANSCRIPTOME(
      new ManifestReaderBuilder(TranscriptomeManifestReader.class).build(), "transcriptome", null),
  SEQUENCE(new ManifestReaderBuilder(SequenceManifestReader.class).build(), "sequence", null),
  READ(new ManifestReaderBuilder(ReadsManifestReader.class).build(), "read", null);
