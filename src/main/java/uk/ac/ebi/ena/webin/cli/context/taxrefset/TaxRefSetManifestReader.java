...
public class TaxRefSetManifestReader extends ManifestReader<TaxRefSetManifest> {
  ...
  public interface Description {
    ...
    String TAXONOMY_SYSTEM =
        "Name of taxonomy system or database from which names are drawn. NCBI taxonomy is preferred.";
    String TAXONOMY_SYSTEM_VERSION = "Version identifier for the taxonomy system or database.";
    String FASTA = "Fasta file";
    String TAB = "TSV file";
    String CUSTOM_FIELD =
        "Custom column in the sequence metadata file. Must be described in format <column name>:<column description>. "
            + "For example, column1:description1. The column names must match exactly the ones used in the sequence metadata file.";
  }

  public TaxRefSetManifestReader(WebinCliParameters parameters, MetadataProcessorFactory factory) {
    super(
        parameters,
        // Fields.
        new ManifestFieldDefinition.Builder()
            ...
            .meta()
            .required()
            .name(Field.TAXONOMY_SYSTEM)
            .desc(Description.TAXONOMY_SYSTEM)
            .and()
            .meta()
            .optional()
            .name(Field.TAXONOMY_SYSTEM_VERSION)
            .desc(Description.TAXONOMY_SYSTEM_VERSION)
            .and()
            .file()
            .required()
            .name(Field.FASTA)
            ...
            .desc(Description.CUSTOM_FIELD)
            .processor(getCustomFieldProcessor())
            .build(),
        // File groups.
    ...
    getCustomFieldProcessor()
        .setCallback(
            (fieldGroup, keyVal) ->
                getManifest(fieldGroup).addCustomField(keyVal.left, keyVal.right));
  }

  private static CustomFieldProcessor getCustomFieldProcessor() {
    if (customFieldProcessor == null) customFieldProcessor = new CustomFieldProcessor();
              ...
              if (getWebinCliParameters() != null) {
                ...
                getFiles(getInputDir(), fieldGroup, Field.TAB)
                    .forEach(
                        tsvFile ->
                            submissionFiles.add(
                                new SubmissionFile(TaxRefSetManifest.FileType.TAB, tsvFile)));
              }
            });
  }
}
