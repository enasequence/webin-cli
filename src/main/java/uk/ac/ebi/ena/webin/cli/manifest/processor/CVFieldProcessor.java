...
public class CVFieldProcessor implements ManifestFieldProcessor {
  public static final CVFieldProcessor CV_BOOLEAN =
      new CVFieldProcessor(new ManifestCVList("yes", "no", "true", "false", "Y", "N"));
    ...
    if (!cvList.contains(value)) {
      result.add(
          ValidationMessage.error(
              WebinCliMessage.CV_FIELD_PROCESSOR_ERROR,
              fieldValue.getName(),
              value,
              cvList.keyList()));
      return;
    ...
    if (!value.equals(corrected)) {
      ...
      result.add(
          ValidationMessage.info(
              WebinCliMessage.CV_FIELD_PROCESSOR_FIELD_VALUE_CORRECTED,
              fieldValue.getName(),
              value,
              corrected));
    }
