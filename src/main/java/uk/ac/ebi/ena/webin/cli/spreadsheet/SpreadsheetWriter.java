...
public class SpreadsheetWriter {
  ...
  public SpreadsheetWriter(SpreadsheetContext spreadsheetContext) {
    ...
    try {
      ...
      throw WebinCliException.systemError(
          ex, "Unable to create spreadsheet: " + spreadsheetContext.getFileName());
    }
  }
  ...
  private XSSFCellStyle getCommonHeaderStyle() {
    ...
    style.setFillForegroundColor(
        new XSSFColor(new byte[] {(byte) 29, (byte) 128, (byte) 134}, null));
  ...
  private void addHeaderText(
      XSSFSheet sheet, Row headerRow, ArrayList<ManifestFieldDefinition> fields) {
    int columnNumber = 0;
    ...
    for (ManifestFieldDefinition field : fields) {
      ...
      cell.setCellStyle(
          field.getRecommendedMinCount() > 0 ? requiredHeaderStyle : optionalHeaderStyle);
  ...
  private void addHeaderComment(
    ...
    for (ManifestFieldDefinition field : fields) {
      ...
      if (field.getType() == ManifestFieldType.META) {
        commentStr =
            creationHelper.createRichTextString(
                field.getDescription()
                    + (field.getRecommendedMinCount() > 0
                        ? " (mandatory field)"
                        : "(optional field)"));
      } else {
  ...
  private List<String> getCvValues(ManifestFieldDefinition field, CVFieldProcessor processor) {
    ...
    if (spreadsheetContext == SpreadsheetContext.READ
        && field.getName().equals(ReadsManifestReader.Field.INSTRUMENT)) {
      excludeValues.add("unspecified");
    }
    return processor.getValues().stream()
      ...
      for (ManifestFieldProcessor processor : field.getFieldProcessors()) {
        if (processor instanceof CVFieldProcessor) {
          DataValidationConstraint constraint =
              helper.createFormulaListConstraint(getCvName(field));
          CellRangeAddressList addressList =
              new CellRangeAddressList(1, lastRowNumber, columnNumber, columnNumber);
  ...
  public void write() {
    try (OutputStream fileOut = new FileOutputStream(spreadsheetContext.getFileName())) {
      ...
      throw WebinCliException.systemError(
          ex, "Unable to write spreadsheet: " + spreadsheetContext.getFileName());
    }
  }
