...
public class TempFileBuilder {
  ...
  public static Path file(String contents) {
    return new TempFileBuilder().contents(contents).build();
  }

  public static Path file(String name, String contents) {
    return new TempFileBuilder().name(name).contents(contents).build();
  }

  public static Path file(Path dir, String contents) {
    return new TempFileBuilder().dir(dir).contents(contents).build();
  }

  public static Path file(Path dir, String name, String contents) {
    return new TempFileBuilder().dir(dir).name(name).contents(contents).build();
  }

  public static Path gzip(String name, String contents) {
    return new TempFileBuilder().name(name).contents(contents).gzip().build();
  }

  public static Path gzip(Path dir, String name, String contents) {
    return new TempFileBuilder().dir(dir).name(name).contents(contents).gzip().build();
  }
                ...
                gzip
                    ? new GZIPOutputStream(
                        Files.newOutputStream(
                            path, StandardOpenOption.TRUNCATE_EXISTING, StandardOpenOption.SYNC))
                    : Files.newOutputStream(
                        path, StandardOpenOption.TRUNCATE_EXISTING, StandardOpenOption.SYNC)));
        os.flush();
        os.close();
